...
public class AutomationTestCases extends BaseClass{
	...
	private void beforeTest() {
		...
		waitUntillPageLoad(5);
		 l=new LoginPage();
		sendKeys(l.getTxtBoxMail(), "ponnagarajchitrambalam@gmail.com");
		sendKeys(l.getTxtBoxPass(), "Naren0150");
		click(l.getBtnLogin());
	}
	
	@Test
	private void the_Application_Should_Allow_User_To_Enter_Corresponding_Data_To_The_Input_Field_In_The_Edit_Candidate_Section() throws InterruptedException {
		 p= new POM_RC01();
		waitUntillWebelementClickable(p.getSideBarCanditateIcon(), 10);
		click(p.getSideBarCanditateIcon());
		waitUntillWebelement(p.getCanditateName(), 5);
		p.getCanditateName().click();
		driver.findElement(By.xpath("//i[text()='edit']")).click();

		sendKeysAfterDeletingExistingText(p.getTxtBoxCanditateMob(), "1234567890");
		
		sendKeysAfterDeletingExistingText(p.getTxtBoxCanditateCity(), "Madurai");
		
		javascriptScrollDownTo(p.getDropDownYearOfExp());

		//actionsClick(p.getDropDownYearOfExp());
		p.getDropDownYearOfExp().click();
		//javascriptClick(p.getDropDownYear_5());
		p.getDropDownYear_5().click();
